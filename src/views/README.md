...
- **Views (Containers):** The main view file (e.g., `TransactionsView.tsx`) is the *only* component that should manage global state, dispatch actions, or interact directly with the global store (`useAppContext`).
- **Extracted Subcomponents (Presentational):** Subcomponents that are extracted into separate files (e.g., `TransactionsTable.tsx`, `CategoryDropdown.tsx`) should be primarily presentational. They **are allowed** to have their own *local presentational state* (e.g., managing whether a modal is open/closed, local form inputs, or UI toggles) as long as it is unrelated to the global data. However, they **must not** manage global state (like the list of transactions, search filters, etc.) and **must not** dispatch actions to the global store directly. Instead, they should inform the parent component about actions the user takes by emitting events via callbacks (like `onDelete`, `onUpdate`), and the top-level component will dispatch the changes to the global state.
